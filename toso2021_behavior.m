...
xlabel('Predictors');
ylabel('\beta');
stem(1:numel(betas),betas,'.k','markersize',25);
...
if tfun(1) == 0
    fig_name = sprintf(...
        'generalization matrix (log) beta1=%.2f, beta2=%.2f',beta_t1,beta_t2);
else
    fig_name = sprintf(...
        'generalization matrix (lin)  beta1=%.2f, beta2=%.2f',beta_t1,beta_t2);
end
fig = figure(figopt,...
    'name',fig_name);
axes(axesopt.default,...
...
if tfun(1) == 0
    fig_name = sprintf(...
        'contraction bias (log) beta1=%.2f, beta2=%.2f',beta_t1,beta_t2);
else
    fig_name = sprintf(...
        'contraction bias (lin)  beta1=%.2f, beta2=%.2f',beta_t1,beta_t2);
end
fig = figure(figopt,...
    'name',fig_name);
axes(axesopt.default,...
...
fig = figure(figopt,...
    'name',sprintf('psychometric curves (w1 = %.2f, w2 = %.2f)',w1,w2));
...
% legend
leg_str = arrayfun(@(x)sprintf('I_2 = %i (mm/s)',x),contrast_set,...
    'uniformoutput',false);
