...
% roi definitions
glm_roi.aroundInitMov = [-glm_roi_span,0] - 400;
glm_roi.preInit = [-glm_roi_span,0];
glm_roi.postInit = [0,glm_roi_span];
glm_roi.preS1Onset = [-glm_roi_span,0];
glm_roi.postS1Onset = [0,glm_roi_span];
glm_roi.preS1Offset = [-glm_roi_span,0];
glm_roi.postS1Offset = [0,glm_roi_span];
glm_roi.preS2Onset = [-glm_roi_span,0];
glm_roi.postS2Onset = [0,glm_roi_span];
glm_roi.preS2Offset = [-glm_roi_span,0];
glm_roi.postS2Offset = [0,glm_roi_span];
glm_roi.preGoCue = [-glm_roi_span,0];
glm_roi.postGoCue = [0,glm_roi_span];
glm_roi.aroundChoiceMov = [0,glm_roi_span] + 400;

% epoch parsing
n_glm = floor((glm_roi_span - glm_win) / glm_step) + 1;
epochs = fieldnames(glm_roi);
n_epochs = numel(epochs);
for ee = 1 : n_epochs
    epoch = epochs{ee};
    glm_time.(epoch) = linspace(...
        glm_roi.(epoch)(1) + glm_win,...
        glm_roi.(epoch)(2) - glm_step,...
        n_glm);
end
...
% preallocation
spkcounts_preS1Onset = nan(n_total_trials,n_glm);
spkcounts_postS1Offset = nan(n_total_trials,n_glm);
spkcounts_preS2Onset = nan(n_total_trials,n_glm);
spkcounts_postS2Offset = nan(n_total_trials,n_glm);
...
for nn = 1 : n_neurons2use
    ...
    for gg = 1 : n_glm
        ...
        % current glm window
        glm_win_onset = glm_roi.aroundInitMov(1) + (gg - 1) * glm_step;
        ...
        % current glm window
        glm_win_onset = glm_roi.preInit(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postInit(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.preS1Onset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postS1Onset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.preS1Offset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postS1Offset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.preS2Onset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postS2Onset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.preS2Offset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postS2Offset(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.preGoCue(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.postGoCue(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        % current glm window
        glm_win_onset = glm_roi.aroundChoiceMov(1) + (gg - 1) * glm_step;
        glm_win_offset = glm_win_onset + glm_win;
        ...
        spkcounts.aroundInitMov(spike_flags,gg) = nansum(spkcounts_aroundInitMov,2);
        spkcounts.preInit(spike_flags,gg) = nansum(spkcounts_preInit,2);
        spkcounts.postInit(spike_flags,gg) = nansum(spkcounts_postInit,2);
        spkcounts.preS1Onset(spike_flags,gg) = nansum(spkcounts_preS1Onset,2);
        spkcounts.postS1Onset(spike_flags,gg) = nansum(spkcounts_postS1Onset,2);
        spkcounts.preS1Offset(spike_flags,gg) = nansum(spkcounts_preS1Offset,2);
        spkcounts.postS1Offset(spike_flags,gg) = nansum(spkcounts_postS1Offset,2);
        spkcounts.preS2Onset(spike_flags,gg) = nansum(spkcounts_preS2Onset,2);
        spkcounts.postS2Onset(spike_flags,gg) = nansum(spkcounts_postS2Onset,2);
        spkcounts.preS2Offset(spike_flags,gg) = nansum(spkcounts_preS2Offset,2);
        spkcounts.postS2Offset(spike_flags,gg) = nansum(spkcounts_postS2Offset,2);
        spkcounts.preGoCue(spike_flags,gg) = nansum(spkcounts_preGoCue,2);
        spkcounts.postGoCue(spike_flags,gg) = nansum(spkcounts_postGoCue,2);
        spkcounts.aroundChoiceMov(spike_flags,gg) = nansum(spkcounts_aroundChoiceMov,2);
    end
end
...
for ee = 1 : n_epochs
    ...
    % preallocation
    betas.(epoch) = zeros(n_neurons2use,n_glm,n_coefficients);
    pvals.(epoch) = zeros(n_neurons2use,n_glm,n_coefficients);
    ...
    for nn = 1 : n_neurons2use
        ...
        for gg = 1 : n_glm
            ...
            mdl = fitglm(Z(trial_flags,:),spkcounts.(epoch)(trial_flags,gg),'linear',...
                'predictorvars',{'prev_choice',s1_lbl,d1_lbl,s2_lbl,d2_lbl,'choice','trial#'},...
                ...'predictorvars',{s1_lbl,d1_lbl,s2_lbl,d2_lbl,'trial#'},...
                'distribution',distro,...
                'intercept',true);
            betas.(epoch)(nn,gg,:) = mdl.Coefficients.Estimate;
            pvals.(epoch)(nn,gg,:) = mdl.Coefficients.pValue;
        end
    end
end
...
xlabel(sprintf('%i-ms non-overlapping epochs',glm_roi_span));
...
for ee = 1 : n_epochs
    ...
    for aa = 1 : n_alphas
        ...
        for bb = 2 : n_coefficients - 1
            ...
            sig_mask = pvals.(epoch)(:,:,bb) < (alphas(aa) / n_glm);
            
            % iterate through signs
            for ss = [-1,1]
                sign_mask = sign(betas.(epoch)(:,:,bb)) == ss;
                n = sum(any(sig_mask & sign_mask,2));
                p = n / n_neurons2use;
                if ss == -1
                    clr = coeff_clrs(1,:);
                else
                ...
                % regressor lines
                if ee == 1 && aa == 2 && ss == 1
                    offset = numel(coeff_str);
                    plot(x*[1,1],[p,yymax*.95-offset*.01],'-k');
                end
            end
