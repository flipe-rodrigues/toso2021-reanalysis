...
%% contrast settings
contrast_str = 'choice_correct';
contrasts = (choice * 2 - 1) .* (correct + 1);
contrast_set = unique(contrasts(valid_flags));
n_contrasts = numel(contrast_set);
contrast_mode_idx = find(contrast_set == mode(contrasts));
contrast_clrs = colorlerp(choice_clrs,conditions.test.n);
contrast_units = 'a.u.';
