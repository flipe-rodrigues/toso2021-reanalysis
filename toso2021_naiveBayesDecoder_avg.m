...
%% slice settings
slice_times = unique([-250;0;t_set(1:end-2)]);
...
% % plot([1,1]*0,ylim,':k');
% % plot(xlim,[1,1]*0,':k');
% % plot([1,1]*0,[0,max(ylim)],'-k',...
% %     'linewidth',1.5);
% % plot([0,max(xlim)],[1,1]*0,'--k',...
% %     'linewidth',1.5);
% 
% % identity line
% plot(xlim,ylim,'--w');
...
axes(...
    ...
    'box','off',...
    'color','w',...
    'xlim',[0,t_set(end-2)],...+[-1,1]*.05*range([0,t_set(end-2)]),...
    'xtick',unique([roi';-500;0;t_set]),...
    'ylim',[0,t_set(end-2)],...+[-1,1]*.05*range([0,t_set(end-2)]),...
    'ytick',unique([roi';-500;0;t_set]));
title('M.A.P.');
