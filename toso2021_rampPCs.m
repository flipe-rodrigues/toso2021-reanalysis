...
s1_tsne = tsne(s1_zpsths');
s1_tsne = tsne(zscore(s1_psths,0,2)');

% compute S2-aligned t-SNE embedding
s2_tsne = tsne(s2_zpsths');
s2_tsne = tsne(zscore(s2_psths,0,2)');
...
plot(si_roi(1)-.025*range(xlim),find(s2_nonramp_flags(s2_theta_idcs)),...
    ...
    'marker','.',...
    'markersize',5);
plot(si_roi(1)-.0125*range(xlim),find(s2_ramp_flags(s2_theta_idcs)),...
    'color',ramp_clrs(1,:),...
    'marker','.',...
    'markersize',5);
...
grapeplot(s1_coeff_neuron(s1_ramp_flags,1),s1_coeff_neuron(s1_ramp_flags,2),...,...
...
grapeplot(s2_coeff_neuron(s2_ramp_flags,1),s2_coeff_neuron(s2_ramp_flags,2),...,...
...
%     'markerfacecolor',s1_cluster_clrs(s1_cluster_flags,:));
grapeplot(s1_score_time(s1_nonramp_flags,1),s1_score_time(s1_nonramp_flags,2),...,...
    'markeredgecolor',s1_cluster_clrs(s1_nonramp_flags,:),...
    'markerfacecolor',s1_cluster_clrs(s1_nonramp_flags,:));
grapeplot(s1_score_time(s1_ramp_flags,1),s1_score_time(s1_ramp_flags,2),...,...
    'markeredgecolor',s1_cluster_clrs(s1_ramp_flags,:),...
    'markerfacecolor',s1_cluster_clrs(s1_ramp_flags,:));
...
% update axis
xxlim = quantile(s1_score_time(:,1),[0,1]+[1,-1]*.01);
yylim = quantile(s1_score_time(:,2),[0,1]+[1,-1]*.01);
xlim(xxlim + [-1,1] * .05 * range(xxlim));
ylim(yylim + [-1,1] * .05 * range(yylim));
set(gca,...
    'xtick',unique([xxlim,0]),...
...
grapeplot(s2_score_time(s2_nonramp_flags,1),s2_score_time(s2_nonramp_flags,2),...,...
    'markeredgecolor',s2_cluster_clrs(s2_nonramp_flags,:),...
    'markerfacecolor',s2_cluster_clrs(s2_nonramp_flags,:)*.85);
grapeplot(s2_score_time(s2_ramp_flags,1),s2_score_time(s2_ramp_flags,2),...,...
    'markeredgecolor',s2_cluster_clrs(s2_ramp_flags,:),...
    'markerfacecolor',s2_cluster_clrs(s2_ramp_flags,:)*.85);
...
% coefficient scatter
grapeplot(s1_tsne(s1_nonramp_flags,1),s1_tsne(s1_nonramp_flags,2),...,...
    'markeredgecolor',s1_cluster_clrs(s1_nonramp_flags,:),...
    'markerfacecolor',s1_cluster_clrs(s1_nonramp_flags,:)*.85);
grapeplot(s1_tsne(s1_ramp_flags,1),s1_tsne(s1_ramp_flags,2),...,...
    'markeredgecolor',s1_cluster_clrs(s1_ramp_flags,:),...
...
% coefficient scatter
grapeplot(s2_tsne(s2_nonramp_flags,1),s2_tsne(s2_nonramp_flags,2),...,...
    'markeredgecolor',s2_cluster_clrs(s2_nonramp_flags,:),...
    'markerfacecolor',s2_cluster_clrs(s2_nonramp_flags,:)*.85);
grapeplot(s2_tsne(s2_ramp_flags,1),s2_tsne(s2_ramp_flags,2),...,...
    'markeredgecolor',s2_cluster_clrs(s2_ramp_flags,:),...
    'markerfacecolor',s2_cluster_clrs(s2_ramp_flags,:)*.85);
...
%%
figure;
s2_cluster_idcs = [s2_upramp_flags | s2_downramp_flags,s2_nonramp_flags] * (1:2)';
silhouette(s2_score_time(:,1:2),s2_ramp_flags);

%%
figure;
% Y = tsne(s2_coeff_neuron);
% Y = tsne(s2_score_time);
Y = tsne(s2_zpsths');
gscatter(Y(:,1),Y(:,2),categorical(s2_ramp_flags,[0,1],{'ramp','non'}),ramp_clrs)
set(gca,axesopt.default);
xlabel('t-SNE 1_1');
ylabel('t-SNE 2_2');
% xlabel('Neuron-wise t-SNE 1 coefficient_1');
% ylabel('Neuron-wise t-SNE 2 coefficient_2');
xxlim = xlim;
yylim = ylim;
xlim(xxlim + [-1,1] * .05 * range(xxlim));
ylim(yylim + [-1,1] * .05 * range(yylim));
