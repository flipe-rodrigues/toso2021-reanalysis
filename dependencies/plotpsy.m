function h = plotpsy(data, psy, plotopt)
    ...
    if plotopt.plotfit
        p = gobjects(3,1);
        first_idx = find(n>0,1,'first');
        last_idx = find(n>0,1,'last');
        xrange = xx >= x(first_idx) & xx <= x(last_idx);
        ...
        p(2) = plot(xx(xx < x(first_idx)),yy(xx < x(first_idx)),...
            'color',plotopt.fitclr,...
            'linestyle',':',...
            'linewidth',plotopt.linewidth,...
            'handlevisibility','off',...
            'parent',plotopt.parent);
        p(3) = plot(xx(xx > x(last_idx)),yy(xx > x(last_idx)),...
            'color',plotopt.fitclr,...
            'linestyle',':',...
            'linewidth',plotopt.linewidth,...
            'handlevisibility','off',...
