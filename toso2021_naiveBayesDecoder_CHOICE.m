...
for ii = 1 : n_contrasts
    contrast_flags = concat_contrasts(nbdopt.test.trial_idcs) == contrast_set(ii);
...
contrast_ref_flags = concat_contrasts(nbdopt.test.trial_idcs) == contrast_set(contrast_mode_idx);
...
for jj = 1 : stim2test_n
    t2_flags = concat_stimuli(nbdopt.test.trial_idcs) == t_set(stim2test_idcs(jj));
    contrast_min_flags = ...
        concat_contrasts(nbdopt.test.trial_idcs) == contrast_set(1) & ...
        t2_flags;
    contrast_max_flags = ...
        concat_contrasts(nbdopt.test.trial_idcs) == contrast_set(end) & ...
        t2_flags;
    title(sps(jj),sprintf('%s: %.2f - %.2f mm/s',...
        contrast_lbl,contrast_set(end),contrast_set(1)));
    p_i2_min = avgfun(P_tR(:,:,contrast_min_flags),3);
    p_i2_min = p_i2_min ./ nansum(p_i2_min,2);
    p_i2_max = avgfun(P_tR(:,:,contrast_max_flags),3);
