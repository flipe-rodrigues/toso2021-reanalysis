...
%         R = X;
        ...
        MAP_ramp(:,bb,mm) = map_ramp;
        MAP_non(:,bb,mm) = map_non;
...
% figure initialization
fig = figure(figopt,...
    'position',[744,400,560,625],...
    'position',[744,400,560,450],...
...
% model selection
model2plot = M;
mus2plot = MUS(:,:,model2plot); % MUS(:,:,model2plot);
lambdas2plot = LAMBDAS(:,:,model2plot);
gammas2plot = GAMMAS(:,:,model2plot);
ramps2plot_flags = RAMP_FLAGS(:,:,model2plot);
...
% update axes
yylim_mu = [0,round(max(ylim(sps(1))) / 10) * 10];
yylim_lambda = [0,round(max(ylim(sps(2))) / 10) * 10];
yylim_gamma = [0,round(max(ylim(sps(3))) / 10) * 10];
set(sps(1),...
    'xlim',mu_bounds,...
    'xtick',mu_bounds,...
    'ylim',yylim_mu,...
...
% point estimate selecting
pthat = 'MAP';
pthat_ramp = eval([pthat,'_ramp']);
pthat_non = eval([pthat,'_non']);

% iterate through models
for mm = 1 : M
    map_ramp_boots = accuracyfun(pthat_ramp(:,:,mm),2);
    map_non_boots = accuracyfun(pthat_non(:,:,mm),2);
    map_ramp_avg = avgfun(map_ramp_boots);
    map_non_avg = avgfun(map_non_boots);
    map_ramp_err = errfun(map_ramp_boots);
    map_non_err = errfun(map_non_boots);
    plot(mm+xoffsets,[map_ramp_avg,map_non_avg],...
        'color','k',...
        'linewidth',1.5);
    errorbar(mm+xoffsets(1),map_ramp_avg,...
        map_ramp_err(1),map_ramp_err(2),...
        'color','k',...
        'marker','o',...
        'markersize',7.5,...
        'markeredgecolor','k',...
        ...
        'linewidth',1.5,...
        'capsize',0);
    errorbar(mm+xoffsets(2),map_non_avg,...
        map_non_err(1),map_non_err(2),...
        'color','k',...
        'marker','o',...
        'markersize',7.5,...
        'markeredgecolor','k',...
...
for mm = 1 : M
    ...
    [~,pval] = kstest2(map_ramp_boots,map_non_boots);
    if pval < .01
        test_str = '**';
    elseif pval < .05
        test_str = '*';
...
% iterate through models
for mm = 1 : M
    sd_ramp_boots = precisionfun(SD_ramp(:,:,mm),2);
    sd_non_boots = precisionfun(SD_non(:,:,mm),2);
