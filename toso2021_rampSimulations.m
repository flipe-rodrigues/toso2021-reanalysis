...
% check what it is that they
% use!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

% iterate through models
for mm = 1 : M
    ramp_boots = std(MU_ramp(:,:,mm),0,2); % mean(SD_ramp(:,:,mm),2);
    non_boots = std(MU_non(:,:,mm),0,2); % mean(SD_non(:,:,mm),2);
    ramp_avg = avgfun(ramp_boots);
    non_avg = avgfun(non_boots);
    ramp_err = errfun(ramp_boots);
    non_err = errfun(non_boots);
...
% iterate through alignments
for mm = 1 : M
    ramp_boots = precisionfun(SD_ramp(:,:,mm),2);
    non_boots = precisionfun(SD_non(:,:,mm),2);
    xx = [-1,1] * .5 / 3 + mm;
    yy = [1,1] * max(yylim);
    plot(xx,yy,...
        'color','k',...
        'linewidth',1.5,...
        'handlevisibility','off');
    [~,pval] = kstest2(ramp_boots,non_boots);
    if pval < .01
        test_str = '**';
    elseif pval < .05
        test_str = '*';
...
%     end
    ...
    errorbar(sps(2),...
        t,nanmean(pthat_non(:,:,mm),2),nanstd(pthat_non(:,:,mm),0,2),...
        'color',ramp_clrs(2,:),...
        'linewidth',.1,...
        'capsize',0);
