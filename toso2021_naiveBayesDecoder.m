...
% stimuli
stim2train_idcs = 1 : n_stimuli; %  [1 + [0,1], n_stimuli + [-1,0]]; % [1 : s2_mode_idx - 1, s2_mode_idx + 1 : n_stimuli];
...
for ii = 1 : n_contrasts
    ...
    pthat_avg = pthat_avgfuns{ff}(nbd.pthat(rr).(type)(:,concat_flags));
    pthat_err = pthat_errfuns{ff}(nbd.pthat(rr).(type)(:,concat_flags));
