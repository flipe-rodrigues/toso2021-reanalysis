...
if strcmpi(task_str,'duration')
    neurons2exclude = [...
        3,8,14,15,16,17,19,20,21,23,29,31,36,45,50,51,59,60,63,66,70,...
        71,79,82,85,86,88,89,92,93,100,102,107,136,137,143,144,153,157,...
        159,160,167,171,181,182,183,184,186,187,188,192,197,198,201,202,...
        212,220,226,239,249,252,253,254,258,261,262,263,265,266,272,273,...
        276,277,297,325,326,340,347,348,351,352,360,361,362,371,374,375,...
        379,382,384,385,394,398,399,402,405,408,412,413,418,429,430,431,...
        432,438,442,444,449,450,451,452,453,467,485,506,507,510,516,517,...
        518,519,530,539,548,556,557,562,572,578,584,585,599,601,602];
...
for nn = neuron_idcs'
    ...
    s2_alignment_flags = ...
        valid_time >= s2_alignment_offset + roi(1) & ...
        valid_time < s2_alignment_offset + t_set(1);
    s2_chunk_flags = ...
        valid_time >= s2_alignment_offset + roi(1) & ...
        valid_time < s2_alignment_offset + roi(2);
    s2_spkrates = s2_spike_rates;
...
fprintf('- minimum sability coefficient: %.2f\n',...
    stability_cutoff);
fprintf('- sorting quality assessed by visual inspection\n');
