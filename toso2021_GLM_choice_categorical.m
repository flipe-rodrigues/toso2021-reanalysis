...
kernel = expkernel('mus',5,'binwidth',1);
i_hist = conv(-prev_i1+prev_i2,kernel.pdf,'same');
t_hist = conv(-prev_t1+prev_t2,kernel.pdf,'same');

%
design_table = table(...X_i1,X_i2,X_t1,X_t2,X_c,X_r,...
    X_i1,X_i2,X_t1,X_t2,...
    X_i1i2,X_t1t2,...,X_t1i1_t,X_t2i2_t...
...
xxticklabels = [...
    '\beta_0',...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{2}(3:end)),x),i_set','uniformoutput',false),...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{3}(3:end)),x),i_set','uniformoutput',false),...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{4}(3:end)),x),t_set','uniformoutput',false),...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{5}(3:end)),x),t_set','uniformoutput',false),...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{6}(3:end)),x),t1t2_set','uniformoutput',false),...
    arrayfun(@(x)sprintf('%s_t=%i',upper(coeff_names{7}(3:end)),x),i1i2_set','uniformoutput',false),...
