...
for rr = 1 : n_runs
    ...
    concat_t2 = nan(n_concats_total,1);
    ...
    for kk = 1 : n_contrasts
        ...
        for ii = 1 : n_t
            t2_flags = concat_t2 == t_set(ii);
            concat_flags = ...
                test_flags & ...
                contrast_flags & ...
                t2_flags;
            neurocurves(kk).x(ii,rr) = (t_set(ii) - min(t_set)) / range(t_set);
...
for ii = 1 : n_t
    ...
    bincounts = histcounts(score(t2_flags),binedges);
    h = histogram(...
        'binedges',binedges,...
...
for kk = 1 : n_contrasts
    ...
    errorbar(t_set,delta_p,err(:,1),err(:,2),...
        'color','k',...
