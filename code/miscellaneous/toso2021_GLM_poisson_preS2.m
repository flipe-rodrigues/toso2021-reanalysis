...
% preallocation
spkcounts_preS2 = nan(n_total_trials,n_glm);
...
for nn = 1 : n_neurons2use
    ...
    for gg = 1 : n_glm
        ...
        spkcounts_preS2(spike_flags,gg) = nansum(spkrates,2);
    end
end
...
n_regressors_preS2 = size(X,2);
n_coefficients_preS2 = n_regressors_preS2 + 1;
    
% preallocation
betas_preS2 = zeros(n_neurons2use,n_glm,n_coefficients_preS2);
pvals_preS2 = zeros(n_neurons2use,n_glm,n_coefficients_preS2);
...
for nn = 1 : n_neurons2use
    ...
    for gg = 1 : n_glm
        ...
        mdl = fitglm(Z(trial_flags,:),spkcounts_preS2(trial_flags,gg),'linear',...
            'predictorvars',{s1_lbl,d1_lbl,s2_lbl,d2_lbl,'trial#'},...
            'distribution',distro,...
            'intercept',true);
        betas_preS2(nn,gg,:) = mdl.Coefficients.Estimate;
        pvals_preS2(nn,gg,:) = mdl.Coefficients.pValue;
    end
end
...
alpha = .01 / bonferroni;
significance_mask = squeeze(pvals_preS2(:,:,2:end)) < alpha;
n_significant = sum(squeeze(nansum(significance_mask,2)) >= 1);

% iterate through coefficients
for bb = 1 : n_regressors_preS2
...
for bb = 2 : n_coefficients_preS2
    ...
    % coefficient map
    coeff_map_preS2.(coeff_lbl) = betas_preS2(:,:,coeff_flags)';
    
    % significance map
    significance_mask_preS2.(coeff_lbl) = ...
        double(pvals_preS2(:,:,coeff_flags)' < .05 / bonferroni) .* sign(coeff_map_preS2.(coeff_lbl)) + ...
        double(pvals_preS2(:,:,coeff_flags)' < .01 / bonferroni) .* sign(coeff_map_preS2.(coeff_lbl));
    
    % pca
    [pc_coeff,~,~,~,exp] = pca(significance_mask_preS2.(coeff_lbl));
    exp_cutoff = 80;
    n_pcs2use = max(sum(cumsum(exp) <= exp_cutoff),2);
    
    % average sorting
    avg_coeffs = mean(coeff_map_preS2.(coeff_lbl));
...
%     diss = pdist(pc_coeff(:,1:n_pcs2use),'euclidean');
    diss = pdist(significance_mask_preS2.(coeff_lbl)','euclidean');
    ...
    width = 2560 / n_regressors_preS2;
    fig = figure(figopt,...
        'position',[541+(bb-2)*width,905,width,1.3248e+03],...
        'name',sprintf('GLM_preS2_significance_%s',...
...
%     significance_mask_preS2.(coeff_lbl) = ...
%         medfilt2(significance_mask_preS2.(coeff_lbl),[3,1]);
    imagesc(glm_roi+[1,0]*glm_win+[1,-1]*glm_step/2,[1,n_neurons2use],...
        significance_mask_preS2.(coeff_lbl)(:,sorted_idcs)',clims);
