...
n_d_pairs = size(d_pairset,1);

% preallocation
p_choice = nan(n_d_pairs,1);
n_trials_perpair = nan(n_d_pairs,1);
...
% annotate NDD = 0
text(.95,.95,sprintf('%s = 0',ndd_lbl),...
    'units','normalized',...
    'color','k',...
    'rotation',45,...
    'fontsize',axesopt.default.fontsize,...
...
for ii = 1 : n_d_pairs
    ...
    xpatch = linspace(d_pairset(ii,1),di_x(idx),1e3);
    ypatch = [repmat(d_pairset(ii,2),1,1e3-1),nan];
    cpatch = repmat(p_choice(ii),1,1e3);
    p(ii) = plot(tfun([d_pairset(ii,1),di_x(idx)]),[1,1]*tfun(d_pairset(ii,2)),...
        'color','k',...
        'linewidth',4.5);
    patch(tfun(xpatch),tfun(ypatch),ypatch,cpatch,...
        'edgecolor','interp',...
        'linewidth',1.5);
    scatter(tfun(di_x(idx)),tfun(d_pairset(ii,2)),...
