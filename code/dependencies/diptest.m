function [h,pvalue,dip] = diptest(x,nboot,alpha)
% calculates Hartigan's DIP statistic and its significance for the
% vector of sample values X
...
boot_dip = nan(nboot,1);
for bb = 1 : nboot
    boot_x = sort(unifrnd(0,1,1,n));
    boot_dip(bb) = dipstat(boot_x);
end
boot_dip = sort(boot_dip);
pvalue = sum(dip < boot_dip) / nboot;
h = pvalue < alpha;

function [dip,xl,xu,ifault,gcm,lcm,mn,mj] = dipstat(x)
...
% as well as the minorant and majorant fits GCM, LCM, and the corresponding
% support indices MN, and MJ
